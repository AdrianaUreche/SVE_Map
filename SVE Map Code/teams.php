...
<?php
	...
	function getpuzzlesforteam($link, $teamid)
	{
		...
		$sql = "SELECT puzzles.pid, name, difficulty, status, url FROM puzzlesolved, puzzles WHERE (puzzles.pid = puzzlesolved.pid) AND (puzzlesolved.teamid = '$teamid')";
	...
	function getquestsforteam($link, $teamid)
	{
		...
		$sql = "SELECT quests.qid, name, difficulty, status, url FROM questsolved, quests WHERE (quests.qid = questsolved.qid) AND (questsolved.teamid = '$teamid')";
...
//		{
		...
		printf("<a href='toggle_%s_status.php?id=%s&from=%s' %s><img alt='%s' title='%s' src='%s' height=40 width=40 align=right></a>", $type, $id, $status, $onclickstring, $statypes[$status], $statypes[$status], $stat_icon);
//	}
//		else
//		{
